...
class GUIView:
    ...
    def buy_security(self):
        ...
        if choice in options:
            ...
            if amount:
                # בקשת רמת סיכון מהמשתמש
                risk_level = simpledialog.askinteger("Risk Tolerance", "Enter your risk tolerance for this purchase (1-10):")
                if risk_level is None or not (1 <= risk_level <= 10):
                    messagebox.showerror("Error", "Invalid risk level.")
                    return

                # בדיקת הלימה בין רמת הסיכון למוצר
                security_risk = options[choice].get_risk_level()
                if security_risk > risk_level:
                    messagebox.showwarning("Risk Warning", f"{options[choice].name} has risk level {security_risk}, which exceeds your tolerance ({risk_level}). Purchase cancelled.")
                    return
